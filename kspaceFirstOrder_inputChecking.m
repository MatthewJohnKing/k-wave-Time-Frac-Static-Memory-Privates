...
elseif flags.elastic_code
    ...
    checkFieldNames(medium, {'sound_speed', 'sound_speed_ref', 'density',...
        'alpha_coeff', 'alpha_power', 'alpha_mode', 'alpha_filter', 'alpha_sign', 'BonA', 'alpha_L'});
...
if isfield(medium, 'alpha_coeff') || isfield(medium, 'alpha_power')
    ...
    if flags.axisymmetric || (isfield(medium, 'alpha_mode') && strcmp(medium.alpha_mode, 'stokes'))
        ...
        if numel(medium.alpha_power) ~= 1 
            if kgrid.dim==1 && numel(medium.alpha_power) == kgrid.Nx
            elseif kgrid.dim==2 && numel(medium.alpha_power) == kgrid.Nx * kgrid.Ny
            elseif kgrid.dim==3 && numel(medium.alpha_power) == kgrid.Nx * kgrid.Ny * kgrid.Nz
            else
                error('medium.alpha_power must be scalar, or the size of the space');
            end
        end

        % check y is real and within 0 to 3
        if ~isreal(medium.alpha_coeff) || (max(max(medium.alpha_power >= 3)) || max(max(medium.alpha_power < 0)))
