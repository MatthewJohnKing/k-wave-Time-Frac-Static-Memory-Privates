...
%% ADJUSTING INTO EXTENDING THE medium.alpha_power and medium.alpha_coeff
% enlarge the alpha grids by extending the edge values into the
% expanded grid

    if numel(medium.alpha_power) > 1
        medium.alpha_power = expandMatrix(medium.alpha_power, expand_size);
    end   
% medium.alpha_coeff is already done for this below
